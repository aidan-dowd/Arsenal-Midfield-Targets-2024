...
```{r, echo=FALSE, out.width='100%', fig.cap="Arsenal will look to strengthen their midfield this summer. What might this midfield trio look like next season?"}
knitr::include_graphics("/Users/aidandowd/Desktop/Arsenal-Mids_Files/Arsenal Midfield.jpeg")
```
...
# Zubimendi <- fb_player_scouting_report(player_url = "https://fbref.com/en/players/3ee0dd59/Martin-Zubimendi", pos_versus = "primary")
# Zubimendi_filtered <- filter(Zubimendi, Versus == "Midfielders")
# Zubimendi_cat <- filter(Zubimendi_filtered, Statistic == "Goals + Assists" | 
...
# Zubimendi_cat <- Zubimendi_cat[c(1,2,3,4,6,7,8,11),]
# 
# Rice <- fb_player_scouting_report(player_url = "https://fbref.com/en/players/1c7012b8/Declan-Rice", pos_versus = "primary")
# Rice_filtered <- filter(Rice, Versus == "Midfielders")
# Rice_cat <- filter(Rice_filtered, Statistic == "Goals + Assists" | 
#                      Statistic == "Progressive Carries" | 
#                      Statistic == "Progressive Passes" | 
#                      Statistic == "Key Passes" | 
#                      Statistic == "Shot-Creating Actions" | 
...
#                      Statistic == "Tackles" | 
#                      Statistic == "Ball Recoveries")
# Rice_cat <- Rice_cat[c(1,2,3,4,6,7,8,11),]

Zubimendi_cat <- data.frame(Player = rep("Martin Zubimendi", 8),
                       Statistic = c("Goals + Assists", "Progressive Carries", "Progressive Passes", "Key Passes", "Shot-Creating Actions", "Tackles", "Interceptions", "Ball Recoveries"),
                       Percentile = c(39,40,62,18,26,36,68,56))

Rice_cat <- data.frame(Player = rep("Declan Rice", 8),
                       Statistic = c("Goals + Assists", "Progressive Carries", "Progressive Passes", "Key Passes", "Shot-Creating Actions", "Tackles", "Interceptions", "Ball Recoveries"),
                       Percentile = c(90,82,90,65,54,59,73,29))

Zubi_fig <- fig <- plot_ly(
    type = 'scatterpolar',
    fill = 'toself'
  ) 
Zubi_fig %>%
  add_trace(
    r = Zubimendi_cat$Percentile,
    theta = Zubimendi_cat$Statistic,
    ...
    add_trace(
    r = Rice_cat$Percentile,
    theta = Rice_cat$Statistic,
    name = 'Declan Rice',
    type = 'scatterpolar',
    fill = 'toself',
...
```{r}
# Zubi_pass <- filter(Zubimendi, StatGroup == "Passing" & scouting_period == "Last 365 Days Men's Big 5 Leagues, UCL, UEL")
# Zubi_pass <- Zubi_pass[c(3,8,11,14,19,22), ]

Zubi_pass <- data.frame(Player = rep("Martin Zubimendi", 6),
                       Statistic = c("Pass Completion %", "Pass Completion % (Short)", "Pass Completion % (Medium)", "Pass Completion % (Long)", "Passes into Final Third", "Progressive Passes"),
                       Percentile = c(69,55,69,74,63,62))

ggplot(Zubi_pass, aes(x = Statistic, y = Percentile, fill = Percentile)) + 
  geom_bar(stat = "identity") + theme_bw() +
  scale_fill_gradient2(low = "white", mid = "red", high = "green", limits = c(0, 100), midpoint = 50) + 
  scale_y_continuous(limits = c(0,100)) + labs(title = "How Does Zubimendi's Passing Stack up Against Other Midfielders", caption = "Data from FBref")
```
...
![](/Users/aidandowd/Desktop/Zubi%20Pos%20O.png)
...
![](/Users/aidandowd/Desktop/Zubi%20D%201.png)
...
![](/Users/aidandowd/Desktop/Zubi%20D%202.png)
...
```{r}
# Neves <- fb_player_scouting_report(player_url = "https://fbref.com/en/players/c2a15a27/Joao-Neves", pos_versus = "primary")
# Neves_filtered <- filter(Neves, Versus == "Midfielders")
# Neves_cat <- filter(Neves_filtered, Statistic == "Goals + Assists" | 
#                      Statistic == "Progressive Carries" | 
#                      Statistic == "Progressive Passes" | 
#                      Statistic == "Key Passes" | 
#                      Statistic == "Shot-Creating Actions" | 
...
#                      Statistic == "Tackles" | 
#                      Statistic == "Ball Recoveries")
# Neves_cat <- filter(Neves_cat, scouting_period == "Last 365 Days Men's Next 14 Competitions")
# Neves_cat <- Neves_cat[c(1,2,3,4,6,7,8,11),]

Neves_cat <- data.frame(Player = rep("João Neves", 8),
                       Statistic = c("Goals + Assists", "Progressive Carries", "Progressive Passes", "Key Passes", "Shot-Creating Actions", "Tackles", "Interceptions", "Ball Recoveries"),
                       Percentile = c(52,54,83,65,73,77,77,96))

Neves_fig <- fig <- plot_ly(
    type = 'scatterpolar',
    fill = 'toself'
  ) 
Neves_fig %>%
  add_trace(
    r = Neves_cat$Percentile,
    theta = Neves_cat$Statistic,
    name = 'Joao Neves',
    type = 'scatterpolar',
    fill = 'toself',
    fillcolor = 'rgba(195, 255, 104);, 1',
...
<center>![](/Users/aidandowd/Desktop/JN%20passing.gif)</center>
...
![](/Users/aidandowd/Desktop/JN%20dribble-mh.png)
...
```{r}
# JN_def <- filter(Neves, StatGroup == "Defense" & scouting_period == "Last 365 Days Men's Next 14 Competitions")
# JN_def <- JN_def[c(1,2,5,8,13), ]

JN_def <- data.frame(Player = rep("João Neves", 5),
                       Statistic = c("% of dribblers tackled", "Interceptions", "Tackles", "Tackles (Att 3rd)", "Tackles Won"),
                       Percentile = c(44,77,77,99,95))

ggplot(JN_def, aes(x = Statistic, y = Percentile, fill = Percentile)) + 
  geom_bar(stat = "identity") + theme_bw() +
  scale_fill_gradient2(low = "white", mid = "red", high = "green", limits = c(0, 100), midpoint = 50) + 
  scale_y_continuous(limits = c(0,100)) + labs(title = "What do Statistics Suggest about João Neves's Defensive Ability?", caption = "Data from FBref")
```
...
```{r}
# BrunoG <- fb_player_scouting_report(player_url = "https://fbref.com/en/players/82518f62/Bruno-Guimaraes", pos_versus = "primary")
# BrunoG_filtered <- filter(BrunoG, Versus == "Midfielders")
# BrunoG_cat <- filter(BrunoG_filtered, Statistic == "Goals + Assists" | 
#                      Statistic == "Progressive Carries" | 
#                      Statistic == "Progressive Passes" | 
#                      Statistic == "Key Passes" | 
#                      Statistic == "Shot-Creating Actions" | 
...
#                      Statistic == "Tackles" | 
#                      Statistic == "Ball Recoveries")
# BrunoG_cat <- BrunoG_cat[c(1,2,3,4,6,7,8,11),]

BrunoG_cat <- data.frame(Player = rep("Bruno Guimaraes", 8),
                       Statistic = c("Goals + Assists", "Progressive Carries", "Progressive Passes", "Key Passes", "Shot-Creating Actions", "Tackles", "Interceptions", "Ball Recoveries"),
                       Percentile = c(90,67,87,81,90,73,55,84))

BrunoG_fig <- fig <- plot_ly(
    type = 'scatterpolar',
    fill = 'toself'
  ) 
BrunoG_fig %>%
  add_trace(
    r = BrunoG_cat$Percentile,
    theta = BrunoG_cat$Statistic,
    name = 'Bruno Guimarāes',
    fill = 'toself',
    fillcolor = '(191, 191, 191);, 1',
...
![](/Users/aidandowd/Desktop/Bruno%20Pass%202.gif)
...
![](/Users/aidandowd/Desktop/Bruno%20Pass.png)
...
knitr::include_graphics(c("~/Desktop/Bruno Press 1.png","~/Desktop/Bruno Press 2.png"))
```
...
![](/Users/aidandowd/Desktop/Bruno%20foul.gif)
...
```{r}
# Frenkie <- fb_player_scouting_report(player_url = "https://fbref.com/en/players/1bacc518/Frenkie-de-Jong", pos_versus = "primary")
# Frenkie_filtered <- filter(Frenkie, Versus == "Midfielders")
# Frenkie_cat <- filter(Frenkie_filtered, Statistic == "Goals + Assists" | 
#                      Statistic == "Progressive Carries" | 
#                      Statistic == "Progressive Passes" | 
#                      Statistic == "Key Passes" | 
#                      Statistic == "Shot-Creating Actions" | 
...
#                      Statistic == "Tackles" | 
#                      Statistic == "Ball Recoveries")
# Frenkie_cat <- Frenkie_cat[c(1,2,3,4,6,7,8,11),]

Frenkie_cat <- data.frame(Player = rep("Frenkie De Jong", 8),
                       Statistic = c("Goals + Assists", "Progressive Carries", "Progressive Passes", "Key Passes", "Shot-Creating Actions", "Tackles", "Interceptions", "Ball Recoveries"),
                       Percentile = c(25,97,96,57,72,69,34,58))

Frenkie_fig <- fig <- plot_ly(
    type = 'scatterpolar',
    fill = 'toself'
  ) 
Frenkie_fig %>%
  add_trace(
    r = Frenkie_cat$Percentile,
    theta = Frenkie_cat$Statistic,
...
```{r}
passing <- fb_big5_advanced_season_stats(season_end_year= 2024, stat_type= "passing", team_or_player= "player")
passing_mf <- filter(passing, Pos == "MF" & Mins_Per_90 >= 10)
passing_mf$ProP_per90 <- passing_mf$PrgP / passing_mf$Mins_Per_90

my_players_passing <- filter(passing_mf, Player == "Declan Rice" |
...
passing_mf$outlier <- ifelse(passing_mf$ProP_per90 >= 9.2 & passing_mf$Cmp_percent_Total >= 90 | passing_mf$Squad == "Arsenal", "Outlier", "Normal")

p <- ggplot(passing_mf, aes(x = ProP_per90, y = Cmp_percent_Total)) + geom_point(alpha = .25, size = 2) + theme_bw() +
  geom_point(data = subset(passing_mf, outlier == "Outlier"), aes(color = outlier), size = 3, color = "red") + labs(title = "How does Frenkie stack up amongst Europe's Elite Passers?", caption = "Data from FBref", y = "Progressive Passes per 90", x = "Total Completion Percentage")

p + geom_text(
  data = subset(passing_mf, outlier == "Outlier" & Player != "Toni Kroos"),
  aes(label = Player),
  vjust = 2.5, 
  hjust = .5,
  size = 3,
...
![](/Users/aidandowd/Desktop/Frenkie%203-mh.png)
...
```{r fig.width=3, fig.show='hold'}
knitr::include_graphics(c("~/Desktop/Frenkie 2.1-mh.png","~/Desktop/Frenkie 2.2-mh.png"))
```
...
![](/Users/aidandowd/Desktop/Frenkie%20pass.gif)
...
![](/Users/aidandowd/Desktop/Frenkie%20Defense%201a.png)
...
![](/Users/aidandowd/Desktop/Frenkie%20Defense%201b.png)
...
```{r}
# Enzo <- fb_player_scouting_report(player_url = "https://fbref.com/en/players/6f806d99/Enzo-Le-Fee", pos_versus = "primary")
# Enzo_filtered <- filter(Enzo, Versus == "Midfielders")
# Enzo_cat <- filter(Enzo_filtered, Statistic == "Goals + Assists" | 
#                      Statistic == "Progressive Carries" | 
#                      Statistic == "Progressive Passes" | 
#                      Statistic == "Key Passes" | 
#                      Statistic == "Shot-Creating Actions" | 
...
#                      Statistic == "Tackles" | 
#                      Statistic == "Ball Recoveries")
# Enzo_cat <- Enzo_cat[c(1,2,3,4,6,7,8,11),]

Enzo_cat <- Frenkie_cat <- data.frame(Player = rep("Enzo Le Fée", 8),
                       Statistic = c("Goals + Assists", "Progressive Carries", "Progressive Passes", "Key Passes", "Shot-Creating Actions", "Tackles", "Interceptions", "Ball Recoveries"),
                       Percentile = c(60,88,88,87,81,85,55,94))

Enzo_fig <- fig <- plot_ly(
    type = 'scatterpolar',
    fill = 'toself'
  ) 
Enzo_fig %>%
  add_trace(
    r = Enzo_cat$Percentile,
    theta = Enzo_cat$Statistic,
    name = 'Enzo Le Fée',
    fill = 'toself',
    fillcolor = '(191, 191, 191);, 1',
    marker = list(color = 'black')
  ...
  ), title = "Enzo Le Fée vs. Declan Rice"
)
```
...
![](/Users/aidandowd/Desktop/EnzoIntel.gif)
...
```{r}
knitr::include_graphics(c("~/Desktop/Enzo Dribble 1-mh.png","~/Desktop/Enzo Dribble 2-mh.png", "~/Desktop/Enzo Dribble 3-mh.png"))
```
...
```{r}
defending <- fb_big5_advanced_season_stats(season_end_year= 2024, stat_type= "defense", team_or_player= "player")
defending_mf <- filter(defending, Pos == "MF" & Mins_Per_90 >= 10)

joined <- merge(defending_mf, passing_mf, on = "Player")
joined$TIper90 <- joined[ ,"Tkl+Int"] / joined$Mins_Per_90




p2 <- ggplot(joined, aes(x = ProP_per90, y = TIper90)) + geom_point(alpha = .25, size = 2) + theme_bw() + geom_point(data = subset(joined, Squad == "Arsenal" | Player == "Enzo Le Fée"), aes(color = outlier), size = 3, color = "red") + labs(title = "How does Enzo rank amongst the best box-to-box midfielders in Europe?", y = "Tackles + Interceptions per 90", x = "Progressive Passes per 90")

p2 + geom_text(
  data = subset(joined, Player == "Enzo Le Fée"),
  aes(label = Player),
  vjust = -1.5, 
  hjust = .5,
  size = 3,
  ...
  data = subset(joined, Squad == "Arsenal"),
  aes(label = Player),
  vjust = -1.5, 
  hjust = .5,
  size = 3,
...
+:================:+:==========:+:=================:+:===============================:+:==============:+
| Martin Zubimendi | 6          | \+ Plays 6 now    | \- Little Desire to leave Spain | 60M            |
|                  |            |                   |                                 |                |
|                  |            | \+ Passing        | \- Physicality                  |                |
+------------------+------------+-------------------+---------------------------------+----------------+
| Joao Neves       | 6          | \+ Ball Retention | \- Phyiscality                  | 80M            |
|                  |            |                   |                                 |                |
|                  |            | \+ Tackling       | \- Inexperience                 |                |
+------------------+------------+-------------------+---------------------------------+----------------+
| Bruno Guimarāes  | 8          | \+ Passing        | \- Defending                    | 80M            |
|                  |            |                   |                                 |                |
|                  |            | \+ Dribbling      | \- Atleticism                   |                |
+------------------+------------+-------------------+---------------------------------+----------------+
| Frenkie De Jong  | 8          | \+ Ball Retention | \- Age                          | 70M            |
|                  |            |                   |                                 |                |
|                  |            | \+ Dribbling      | \- Defensive Awarness           |                |
+------------------+------------+-------------------+---------------------------------+----------------+
| Enzo Le Fee      | 8          | \+ Intelligence   | \- G/A Output                   | 25-30M         |
|                  |            |                   |                                 |                |
|                  |            | \+ Tackling       | \- Competition Level            |                |
+------------------+------------+-------------------+---------------------------------+----------------+
